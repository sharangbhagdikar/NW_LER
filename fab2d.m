...
noise2 = wgn(Fs,Fr,10*log10(lambda^2));
fnoise = fft(noise)/n;
fnoise2 = fft(noise2)/n;

for i = 1:Fr
   
    a = conv(fnoise(:,i),facf);
    a = n*ifft(a);
    b = conv(fnoise2(:,i),facf);
    b = n*ifft(b);
    frx = real(a)*cos(theta(i)) + xm(i,1) + real(b)*sin(theta(i));          % Real part??
    fry = real(a)*sin(theta(i)) + xm(i,2) + real(b)*cos(theta(i));
